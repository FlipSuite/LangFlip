...
def generate_speech(sentence, root_path, index, voice_audio_file):
    ...
    # generate_bark_speech(sentence, resources_path, index, voice_audio_file)5

    generate_openvoice_speech(
        sentence, en_speaker_path, converter_path, root_path, voice_audio_file, index
    )

    save_path = os.path.join(root_path, "data_management", f"{index}.wav")

    return save_path


def generate_bark_speech(sentence, resources_path, index, voice_audio_file):
    ...
    inputs = processor(
        "Bonjour je m'appele rémy et je test bark pour voir si ça marche",
...
def generate_openvoice_speech(
    ...
    tone_color_converter = ToneColorConverter(
        f"{converter_path}/config.json", device=device
    )
    ...
    target_se, audio_name = se_extractor.get_se(
        voice_audio_file, tone_color_converter, target_dir="processed", vad=True
    )

    save_path = f"{reference_voice_clips_location}/{index}.wav"

    # Run the base speaker tts
    src_path = f"{reference_voice_clips_location}/tmp.wav"
    base_speaker_tts.tts(
        sentence, src_path, speaker="default", language="English", speed=1.0
    )
